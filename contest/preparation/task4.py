...
for ind in range(0,Q):
    traitors.append([t_salaries[ind], t_degree[ind], t_exclusive[ind]])
t_salaries.clear()
t_degree.clear()
t_exclusive.clear()

for t_ind in range(0,Q):
    flag = True
    for c_ind in range(0,N):
        if (traitors[t_ind][2] == c_ind+1 and country[c_ind][2]==1) or (traitors[t_ind][0] >= country[c_ind][0] and traitors[t_ind][1] >= country[c_ind][1]):
            print(c_ind+1,end=" ")
            flag = False
            break
